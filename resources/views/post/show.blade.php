...
@section('contents')
    ...
    <div class="mb-4">
        Categoria: <span class="badge bg-warning">{{ $posts->categoria->descricao }}</span>
    </div>

    <div>
        {!! nl2br(strip_tags($posts->conteudo, '<p><br><string>')) !!}
    </div>

    <div class="text-muted text-end mt-4">
        Criado por:
        {{ $posts->autor?->name ?? 'Desconhecido' }} -
        Última atualização
        {{ $posts->updated_at?->format('d/m/Y H:i:s') ?? $posts->created_at?->format('d/m/Y H:i:s') }}
    </div>
